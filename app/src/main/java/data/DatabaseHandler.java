...
public class DatabaseHandler extends SQLiteOpenHelper {
    ...
    public ArrayList<Meal> getMeals() {
        ...
        Cursor cursor = dba.query(Constants.TABLE_NAME,
                new String[]{Constants.KEY_ID, Constants.MEAL_NAME, Constants.MEAL_CALORIES_NAME,
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                Meal meal = new Meal();
                meal.setMealName(cursor.getString(cursor.getColumnIndex(Constants.MEAL_NAME)));
                meal.setCalories(cursor.getInt(cursor.getColumnIndex(Constants.MEAL_CALORIES_NAME)));
                meal.setMealId(cursor.getInt(cursor.getColumnIndex(Constants.KEY_ID)));


                DateFormat dateFormat = DateFormat.getDateInstance();
                String date = dateFormat.format(new Date(cursor.getLong(cursor.getColumnIndex(Constants.DATE_NAME))).getTime());

                meal.setRecordDate(date);
